...
mat$Signature <- factor(paste0(mat$Signature, " "), levels = paste0("Signature",6:1," "))



### Plot Heatmap Rank 6 ###
p1 <- ggplot(mat, aes(x = variable, y = 1, fill = subtype)) + geom_tile(color = "gray") +
...
p2 <- ggplot(mat, aes(x = variable, y = 1, fill = signature_assign)) + geom_tile(color = "gray") +
    theme_void() + scale_fill_manual("Assigned\nSignature", values = pal) +
    theme(axis.title.y = element_text(size=12)) + labs(y = "                 ")

p3 <- ggplot(mat, aes(x = variable, y = Signature, fill = value)) + geom_tile(color = "gray") +
...
png("Signatures/results/figures/RNA_rank6_heatmap.png", width = 10, height = 4, res = 600, units = "in")
