...
waterfall_TR <- function(df, arche, drug, TR, plot.indiv = F) {
    ...
    # print out waterfall plot if needed
    if (plot.indiv == TRUE) {
        path = paste0("DrugResponsePDX/results/figures/indiv_plots/", TR, "_waterfall_", arche, "_", drug, ".png")
        png(path, width=175, height=125, units='mm', res = 600, pointsize=80)
        print(p)
        dev.off()
    }
...
scatter_TR <- function(df, arche, drug, TR, combiations, i, plot.indiv = F) {
    ...
    # print out plot if needed
    if (plot.indiv == TRUE) {
        path = paste0("DrugResponsePDX/results/figures/indiv_plots/", TR, "_scatter", arche, "_", drug, ".png")
        png(path, width=125, height=125, units='mm', res = 600, pointsize=80)
        print(p)
        dev.off()
    }
