...
source("utils/plots/plot_all_drug_corr.R")
...
# drug sensitivity processed from ARCHE_CCLs.R
...
# helper function to search for BCa drug overlap
drug_overlap <- function(sen, label) {
    ...
    for (i in 1:length(bca_drugs)) {
        drug = bca_drugs[i]
        if (drug %in% tested_drugs) df$Present[i] <- "Present"
    }
