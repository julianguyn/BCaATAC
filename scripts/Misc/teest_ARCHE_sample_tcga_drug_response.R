...
# deviations
deviat_cells <- get_scores(paste0("data/rawdata/all_scoring/cell_tcga.Deviations.txt"), c_meta)
...
deviat_t <- get_scores(paste0("data/rawdata/all_scoring/pdx_tcga.Deviations.txt"), p_meta_t)
normdv_t <- znorm(deviat_t)
deviat_k <- get_scores(paste0("data/rawdata/all_scoring/pdx_tcga.Deviations.txt"), p_meta_k)
...
plot_PDX_ARCHEs <- function(compile, label) {
    ...
    for (arche in paste0("ARCHE", 1:6)) {
        ...
        filename <- paste0("data/results/figures/Misc/all_scoring/pdx_tcga/drugresponse/PDXs_", arche, "_", label, ".png")
