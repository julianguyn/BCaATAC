...
# load in arche zscores and normalize
zscore_t <- get_arche_scores("pdxs", "data/rawdata/TCGA_bg/PDXs_50k.Zscore.txt", p_meta_t)
normzs_t <- znorm(zscore_t)
zscore_k <- get_arche_scores("pdxs", "data/rawdata/TCGA_bg/PDXs_50k.Zscore.txt", p_meta_k)
normzs_k <- znorm(zscore_k)

# load in arche deviations and normalize
deviat_t <- get_arche_devs("PDXs", "data/rawdata/TCGA_bg/PDXs_50k.Deviations.txt", p_meta_t)
normdv_t <- znorm(deviat_t)
deviat_k <- get_arche_devs("PDXs", "data/rawdata/TCGA_bg/PDXs_50k.Deviations.txt", p_meta_k)
...
# get samples above threshold
zscore_t_sumdev <- get_devs(zscore_t, "zscore_pdx_t", 55, 75)
zscore_k_sumdev <- get_devs(zscore_k, "zscore_pdx_k", 55, 75)
deviat_t_sumdev <- get_devs(deviat_t, "deviat_pdx_t", 0.25, 0.4)
deviat_k_sumdev <- get_devs(deviat_k, "deviat_pdx_k", 0.25, 0.4)
...
dir <- "data/results/figures/Misc/TCGA_bg/"
...
# last save: April 24, 2026
save(cell_toPlot, pdx_all_toPlot, pdx_sumdev_toPlot, file = "data/results/data/Misc/ARCHE_drug_response_testing_TCGA_bg.RData")
...
plot_PDX_ARCHEs <- function(compile, label) {
    ...
    for (arche in paste0("ARCHE", 1:6)) {
        ...
        filename <- paste0("data/results/figures/Misc/TCGA_bg/PDXs_", arche, "_", label, ".png")
